...
catcher_pos = 0
catcher_color = (1, 1, 1) # White Color
catcher_info = [{
...
def mouseListener(button, state, x, y):
    ...
    if button==GLUT_LEFT_BUTTON:
        ...
        #### Handle diamond position do not overlap with buttons
...
def animate():
    ...
    if stop and isfrozen != True:    
         ...
         ##### Create another diamond for generating simultanious diamond falling
                diamond_x = random.randint(20, 500) 
                if diamond_x <= arrow_box['x'] + arrow_box["width"]:
                    diamond_x = 65
                elif diamond_x >= cross_box['x']:
